...
func (db *Database) rebuild(ctx context.Context) error {
	...
	for it.Next() {
		...
		if n.hasValue() {
			if err := currentView.Insert(ctx, path.AsKey(), n.value.value); err != nil {
				return err
			}
			currentViewSize++
		}
...
func (db *Database) GetChangeProof(
	...
	// order.
	changedPaths := maps.Keys(changes.values)
	slices.SortFunc(changedPaths, func(i, j Path) bool {
		return i.Compare(j) < 0
	})

	// TODO: sync.pool these buffers
	result.KeyValues = make([]KeyValue, 0, len(changedPaths))
	result.DeletedKeys = make([][]byte, 0, len(changedPaths))

	for _, path := range changedPaths {
		change := changes.values[path]
		key := path.AsKey()
...
func (db *Database) getKeyValues(
	...
	for remainingLength > 0 && it.Next() {
		...
		if keysToIgnore.Contains(NewPath(key)) {
			continue
		}
